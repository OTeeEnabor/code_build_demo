...
```
aws s3 mb s3://aws-opendelta-devops --region eu-west-1 --profile boto3_1
aws s3api put-bucket-versioning --bucket aws-opendelta-devops --versioning-configuration Status=Enabled --region eu-west-1 
```

# deploy the files into S3
# https://docs.aws.amazon.com/cli/latest/reference/deploy/push.html
```
aws deploy push --application-name 	fraud-app --s3-location s3://aws-opendelta-devops/	fraud-app/app.zip --ignore-hidden-files --region eu-west-1 
```
...
# https://docs.aws.amazon.com/cli/latest/reference/s3api/put-bucket-versioning.html
```
aws s3 mb s3://aws-devops-opendelta --region eu-west-1
aws s3api put-bucket-versioning --bucket aws-devops-opendelta --versioning-configuration Status=Enabled --region eu-west-1
```

# deploy the files into S3
# https://docs.aws.amazon.com/cli/latest/reference/deploy/push.html
```
aws deploy push --application-name MyDeployDemo --s3-location s3://aws-devops-opendelta/MyDeployDemo-test/app.zip --ignore-hidden-files --region eu-west-1
```